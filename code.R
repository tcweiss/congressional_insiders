...
scrape_house <- function(path) {
  ...
  # If report is handwritten, return tibble with one row.
  if(all(nchar(text)) == 0) {
    
    data <- tibble("transaction_date" = 'HANDWRITTEN',
                   "ticker" = 'HANDWRITTEN',
                   "asset" = 'HANDWRITTEN',
                   "type" = 'HANDWRITTEN',
                   "amount" = 'HANDWRITTEN',
                   ...
                   "id" = 'HANDWRITTEN',
                   "filing_status" = 'HANDWRITTEN') %>% 
      mutate("is_stock" = "HANDWRITTEN")
    
    # If not handwritten but no security abbreviations, use function for old
    # forms and add variable indicating whether asset is known to be a stock.
  } else if (!any(grepl('\\[[A-Z]{2}\\]', text, ignore.case = TRUE))) {
...
# 'ptr_link', 'name', 'disclosure_date', transaction_date, 'owner', 'ticker',
...
scrape_senate_all <- function(start_date, end_date, driver) {
  ...
  while(i <= pages) {
    ...
    ptr_link <- read_html(source) %>% 
      html_nodes(xpath = '//tbody') %>% 
      html_children() %>% 
      html_nodes(., 'a')
    ptr_link <- bind_rows(lapply(xml_attrs(ptr_link), function(x) data.frame(as.list(x), stringsAsFactors=FALSE)))['href'] %>% 
      as_tibble() %>% 
      mutate(href = paste0('https://efdsearch.senate.gov', href))
    ptr_link <- as.vector(ptr_link$href)
    ...
    result <- tibble("ptr_link" = ptr_link,
                     "first" = first,
                     "last" = last,
                     "disclosure_date" = disclosure_date) %>% 
      mutate("name" = paste(first, last)) %>% 
      select(ptr_link, name, disclosure_date)
  ...
  for(i in 1:nrow(results)) {
    ...
    # Go to page.
    remDr$navigate(results$ptr_link[i])
    Sys.sleep(4)
    ...
    if(length(transaction_date) == 0) {
      ...
      trades <- rbind(trades, tibble("ptr_link" = results$ptr_link[i],
                                     "asset" = "HANDWRITTEN",
                                     "ticker" = "HANDWRITTEN",
                                     "owner" = "HANDWRITTEN",
                                     "type" = "HANDWRITTEN",
      ...
      trades <-  rbind(trades, tibble("ptr_link" = rep(results$ptr_link[i], n_trans),
                                      "asset" = asset,
                                      "ticker" = ticker,
                                      "owner" = owner,
                                      "type" = type,
