...
- `scrape_house_year()` scrapes trades of all Representatives from all reports filed in a given year
- `scrape_senate_year()` scrapes trades of all Senators between a specified starting and ending date
...
Congressional stock trades are disclosed so-called *periodic transaction reports* (PTRs). The House of representatives publishes them as PDFs [here](https://disclosures-clerk.house.gov/FinancialDisclosure). To extract all public stock trades from a PTR, use `scrape_house()`. The only argument is the URL pointing the PTR (or the path, if you saved the file saved locally):
...
The Senate publishes PTRs on [this](https://efdsearch.senate.gov/search/) page. This is a non-static webpage, and the URL do not follow some predictable pattern as for the House, so the scraping approach works differently. To scrape all Senate trades filed over a certain period, use `scrape_senate_year()`. The arguments are the starting and ending date (format MM/DD/YYYY) and a version of Chromedriver that matches your installed version of Chrome:
...
Running this function will open a window in Google chrome. The function automatically controls the window, so do not close it until it the function has run. Scraping a period of one year should take less time than for the House. The function will return a tibble with the following variables
...
7) Finally, I slightly adjusted the names of politicians to be in line with those listed in other databases (e.g. "Jim Inhofe" was changed to "James M Inhofe"). This means first and middle names might be differnt from those in reports. 
